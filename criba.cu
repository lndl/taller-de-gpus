...
const int N = 2097152;    // Número predeterminado de elementos en los vectores
...
//  kernel
__global__ void kernel_criba(basetype *const vector,   const int n){
 	unsigned long int global_id = blockIdx.x * blockDim.x + threadIdx.x;
	//verificamos si number es primo dividiendo por todos los anteriores
	unsigned int number = vector[global_id];
...
//			break;
		...
		vector[global_id] = number;
	}

}

//  Función para sumar dos vectores en la GPU
void criba_GPU( basetype arrayV[], const unsigned int n, const unsigned int blk_size){
	double timetick;
	...
	timetick = dwalltime();
	kernel_criba<<<dimGrid, dimBlock>>>(cV, n);
...
int main(int argc, char *argv[]){
	...
	timetick = dwalltime();
	basetype *vector = (basetype *) malloc(numBytes); // Vector con datos de entrada
	init_CPU_array(vector, n);
